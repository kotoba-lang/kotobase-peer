(ns kotobase-peer.projection-datalog-test
  "kotobase-projection's differential-maintenance tests, run against a real store.

  They live HERE rather than in kotobase-projection because that is the only
  direction without a cycle. projection is a dependency of this repo
  (`atomic-publication` is a facade over it), and these tests need a datom store
  to maintain queries against -- so keeping them there meant projection
  depending on peer for its tests while peer depends on projection for its code.

  A cycle cannot be pin-consistent: each side can only name a slightly older
  revision of the other. Measured 2026-08-18, pointing either side at the
  other's current main pushed the conflicting-pin count UP, eight pairs to
  seventeen, because the traversal re-enters at a new version each way round.
  Moving the tests removes the edge instead of trying to align it.

  Nothing is weakened: they still run against `peer/transact`, `peer/query` and
  `peer/transact-effective`, which is what they always proved projection
  against. A test double would have been the weaker option -- it proves
  projection against a model of a store rather than the store."
